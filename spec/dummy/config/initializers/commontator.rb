# ############################################################
#
# See https://github.com/lml/commontator for more information
#
# ############################################################
...
Commontator.configure do |config|
  ...
  config.current_user_proc = lambda { |controller| controller.current_spree_user }
  ...
  config.user_name_proc = lambda { |user| "#{user.try(:firstname)} #{user.try(:lastname)}".strip }
  ...
  config.user_avatar_proc = lambda { |user, view|
    if user && user.images.any?
      view.image_tag(
        user.images[0].attachment.url(:small), size: '60')
    else
       view.image_tag(
         "frontend/avatar.png", size: '60')
    end
   }
  ...
  # Any other value requires the paginate gem
  ...
  config.commontable_name_proc = lambda { |thread|
    commontable_instance = thread.commontable
    case commontable_instance.class.to_s
    when "Spree::Order"
      if commontable_instance.purchase_order?
        name = "Purchase Order #{commontable_instance.try(:po_display_number)}"
      else
        name = "Order #{commontable_instance.display_number}"
      end
    when "Spree::Account"
      name = "Account \'" + commontable_instance.fully_qualified_name + "\'"
    end
  }
  ...
  config.comment_url_proc = lambda { |comment, app_routes|
      commontable_instance = comment.thread.commontable
      case commontable_instance.class.to_s
      when "Spree::Order"
        if commontable_instance.purchase_order?
          url = "/manage/purchase_orders/#{commontable_instance.number}/edit"
        else
          url = "/manage/orders/#{commontable_instance.number}/edit"
        end
      when "Spree::Account"
        url = "/manage/customers/" + commontable_instance.fully_qualified_name.downcase.gsub(/\s+/, "-") + "/accounts/" + commontable_instance.id.to_s
      end

      if Rails.env.development?
        local_port = (ENV['PORT'] || '3000')
        url = ENV['DEFAULT_URL_HOST'] + ":" + local_port + url;
      else
        url = ENV['DEFAULT_URL_HOST'] + url;
      end
    }
  ...
  #current_user.class.where('username LIKE ?', "#{query}%")     current_user.class.where('company_id = 1').first.firstname
  config.user_mentions_proc = lambda { |current_user, query|
    admin_ids = Spree::User.joins(:spree_roles).where('spree_roles.name = ?','admin').ids
    current_user.company.users
                .where.not('spree_users.id in (?)', admin_ids)
                .where('lower(firstname) LIKE ? or lower(lastname) LIKE ?', "#{query}%".downcase, "#{query}%".downcase)
  }
end
