module Spree::QbdIntegration::Action::Variant::ItemNonInventory

  def qbd_variant_to_item_non_inventory_xml(xml, variant_hash, match = nil)
    if match
    ...
    # if 'item_non_inventory_sales_and_purchase'
    if variant_hash.fetch(:variant_type) == 'non_inventory_item' && variant_hash.fetch(:product).fetch(:for_sale) && variant_hash.fetch(:product).fetch(:for_purchase)
      # SalesAndPurchase
    ...
    elsif variant_hash.fetch(:variant_type) == 'non_inventory_item'
      # SalesOrPurchase
  ...
  def qbd_item_non_inventory_xml_to_variant(response, variant_hash)
    ...
    xpath_base = '//QBXML/QBXMLMsgsRs/ItemNonInventoryQueryRs/ItemNonInventoryRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("ItemNonInventoryRet", {})
    qbd_item_non_inventory_hash_to_variant(qbd_hash, variant.try(:product), variant)

    # Need to return a hash because calling method is expecting hash of attributes to update
    return {}
  end

  def qbd_all_associated_matched_item_non_inventory?(response, object_hash)
    xpath_base = '//QBXML/QBXMLMsgsRs/ItemNonInventoryQueryRs/ItemNonInventoryRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("ItemNonInventoryRet", {})
  ...
  def qbd_item_non_inventory_create_variant_sub_steps(qbd_hash)
    ...
    if self.reload.current_step.try(:sub_steps).try(:incomplete).blank?
      qbd_item_non_inventory_hash_to_variant(qbd_hash)
    end
  end

  def qbd_item_non_inventory_hash_to_variant(qbd_hash, product = nil, variant = nil)
    by_name = self.integration_item.qbd_match_with_name
    if product.nil?
      if qbd_hash.fetch('ParentRef', nil)
        parent_match = self.integration_item.integration_sync_matches.where(
    ...
    product.product_type = 'non_inventory_item'
    variant.variant_type = 'non_inventory_item'
    ...
    desc = qbd_non_inventory_item_find_or_assign_associated_opts(qbd_hash, variant, product)
    if by_name
  ...
  def qbd_non_inventory_item_find_or_assign_associated_opts(qbd_hash, variant = nil, product = nil)
  ...
  def qbd_item_non_inventory_to_csv(csv, item)
