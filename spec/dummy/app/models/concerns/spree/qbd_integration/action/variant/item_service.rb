module Spree::QbdIntegration::Action::Variant::ItemService

  def qbd_variant_to_item_service_xml(xml, variant_hash, match = nil)
    if match
    ...
    if variant_hash.fetch(:variant_type) == 'service' && variant_hash.fetch(:product).fetch(:for_sale) && variant_hash.fetch(:product).fetch(:for_purchase)
      # SalesAndPurchase
    ...
    elsif variant_hash.fetch(:variant_type) == 'service'
      # SalesOrPurchase
  ...
  def qbd_item_service_xml_to_variant(response, variant_hash)
    ...
    xpath_base = '//QBXML/QBXMLMsgsRs/ItemServiceQueryRs/ItemServiceRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("ItemServiceRet", {})
    qbd_item_service_hash_to_variant(qbd_hash, variant.try(:product), variant)
    # Need to return a hash because calling method is expecting hash of attributes to update
    return {}
  end

  def qbd_all_associated_matched_item_service?(response, object_hash)
    xpath_base = '//QBXML/QBXMLMsgsRs/ItemServiceQueryRs/ItemServiceRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("ItemServiceRet", {})

    qbd_service_find_or_assign_associated_opts(qbd_hash)

    self.reload.current_step.try(:sub_steps).try(:incomplete).blank?
  end

  def qbd_item_service_create_variant_sub_steps(qbd_hash)
    if qbd_hash.fetch('ParentRef', nil)
      parent_match = self.integration_item.integration_sync_matches.where(
        integration_syncable_type: "Spree::Variant",
        sync_type: "ItemService",
    ...
    if self.reload.current_step.try(:sub_steps).try(:incomplete).blank?
      qbd_item_service_hash_to_variant(qbd_hash)
    end
  end

  def qbd_item_service_hash_to_variant(qbd_hash, product = nil, variant = nil)
    by_name = self.integration_item.qbd_match_with_name
    if product.nil?
      if qbd_hash.fetch('ParentRef', nil)
        parent_match = self.integration_item.integration_sync_matches.where(
    ...
    desc = qbd_service_find_or_assign_associated_opts(qbd_hash, variant, product)
    if by_name
  ...
  def qbd_service_find_or_assign_associated_opts(qbd_hash, variant = nil, product = nil)
  ...
  def qbd_item_service_to_csv(csv, item)
