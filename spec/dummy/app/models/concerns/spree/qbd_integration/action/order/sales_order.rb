module Spree::QbdIntegration::Action::Order::SalesOrder

  def qbd_order_to_sales_order_xml(xml, order_hash, match = nil)
    if match
      self.qbd_update_order_to_sales_order_xml(xml, order_hash, match)
    else
      self.qbd_new_order_to_sales_order_xml(xml, order_hash, nil)
    end
  end

  def qbd_new_order_to_sales_order_xml(xml, order_hash, match = nil)
    ...
    if self.integration_item.qbd_sales_order_template.present?
      xml.TemplateRef do
        xml.FullName       self.integration_item.qbd_sales_order_template
      end
    end
  ...
  def qbd_update_order_to_sales_order_xml(xml, order_hash, match = nil)
      ...
      if BUNDLE_TYPES.has_key?(line_item.fetch(:item_type))
        xml.SalesOrderLineGroupMod do
          ...
          line_item.fetch(:parts_variants, []).each do |part_variant|
            ...
            part_line_match = nil
            qbd_sales_order_line_mod(xml, order_hash, part_variant, part_match)
          end
        ...
        qbd_sales_order_line_mod(xml, order_hash, line_item, variant_match, line_item_match)
      end
    end
  ...
  def qbd_sales_order_line_mod(xml, order_hash, line_item, variant_match, line_item_match = nil)
    xml.SalesOrderLineMod do
  ...
  def qbd_order_to_void_sales_order_xml(xml, order_hash, match = nil)
    xml.TxnVoidType   'SalesOrder'
    xml.TxnID         match.try(:sync_id)
  end

  def qbd_sales_order_xml_to_order(response, order_hash)
    order = self.company.sales_orders.find_by_id(order_hash.fetch(:id))
    xpath_base = '//QBXML/QBXMLMsgsRs/SalesOrderQueryRs/SalesOrderRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("SalesOrderRet", {})
    qbd_sales_order_hash_to_order(qbd_hash, order)
    # Need to return a hash because calling method is expecting hash of attributes to update
    return {}
  end

  def qbd_all_associated_matched_sales_order?(response, object_hash)
    xpath_base = '//QBXML/QBXMLMsgsRs/SalesOrderQueryRs/SalesOrderRet'
    qbd_hash = Hash.from_xml(response.xpath(xpath_base).to_xml).fetch("SalesOrderRet", {})

    qbd_sales_order_find_or_assign_associated_opts(qbd_hash)

    self.reload.current_step.try(:sub_steps).try(:incomplete).blank?
  end

  def qbd_sales_order_create_order_sub_steps(qbd_hash)
    qbd_sales_order_find_or_assign_associated_opts(qbd_hash)
    if self.reload.current_step.try(:sub_steps).try(:incomplete).blank?
      qbd_sales_order_hash_to_order(qbd_hash)
    end
  end

  def qbd_sales_order_hash_to_order(qbd_hash, order = nil)
    if order.nil?
    ...
    transaction do
      qbd_sales_order_find_or_assign_associated_opts(qbd_hash, order)
  ...
  def qbd_sales_order_find_or_assign_associated_opts(qbd_hash, order = nil)
    ...
    [qbd_hash.fetch('SalesOrderLineRet', [])].flatten.each do |txn_line|
      begin
    ...
    if self.integration_item.try(:qbd_ungroup_grouped_lines)
      [qbd_hash.fetch('SalesOrderLineGroupRet', [])].flatten.each do |group_line|
        [group_line.fetch('SalesOrderLineRet', [])].flatten.each do |txn_line|
          begin
            item_name = txn_line.fetch('ItemRef', {}).fetch('FullName', nil)
            item_name ||= txn_line.fetch('ItemRef', {}).fetch('Name', nil)
            next if item_name.blank?
    ...
    if order.present?
      ...
      if account.nil?
        raise "Unable to find customer #{qbd_hash.fetch('CustomerRef', {}).fetch('FullName',nil)} for SalesOrder RefNumber: #{qbd_hash.fetch('RefNumber', nil)}"
      end
  ...
  def qbd_sales_order_query_xml(xml)
