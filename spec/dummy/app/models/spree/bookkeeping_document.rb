# == Schema Information
#
# Table name: spree_bookkeeping_documents
#
...
module Spree
  class BookkeepingDocument < ActiveRecord::Base
    ...
    #
    def pdf
      if self.printable.vendor.store_pdf
        send_or_create_pdf
      else
        render_pdf
      end
    ...
    #
    def file_name
      @_file_name ||= "#{template}-D#{id}-#{number}.pdf"
    end
    ...
    #
    def storage_path
      storage_path = Rails.root.join(self.printable.vendor.storage_path, template.pluralize)
