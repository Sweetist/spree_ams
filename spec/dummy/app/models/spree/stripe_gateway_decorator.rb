...
Spree::Gateway::StripeGateway.class_eval do
    ...
    def create_profile(payment)
      ...
      # NOTE: removing email field until we have ability to edit profile via API
      options = {
        # email: payment.order.try(:account).try(:valid_emails_string),
        email: nil,
      ...
      if source.number.blank? && source.gateway_payment_profile_id.present?
        ...
        begin
          token = Stripe::Token.create(
            ...
            api_key: preferred_publishable_key
          )
        rescue Exception => e
          raise Spree::Core::GatewayError.new(e.message)
        end
        unless token.try(:id).present?
          raise Spree::Core::GatewayError.new("Failed to tokenize card.")
        end
        creditcard = token.try(:id)
      end
    ...
    def options_for_purchase_or_auth(money, creditcard, gateway_options)
      options = {}
      options[:description] = "Sweet Order ID: #{gateway_options[:order_id]}"
    ...
    def address_for(payment)
      {}.tap do |options|
        if address = payment.account.try(:bill_address)
