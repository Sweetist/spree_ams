...
total_row = []
total_row << 'Total'
total_row << '' if view_settings.variant_sku
total_row << '' if view_settings.line_item_lot_number
total_row << '' if view_settings.variant_pack_size
total_row << '' if vendor.try(:include_unit_weight)
total_row << @doc.printable.item_count
total_row << @doc.printable.display_total_weight_in(vendor.weight_units) if vendor.try(:include_total_weight)
data += [total_row]
...
pdf.table(data, header: true, position: :center, column_widths: column_widths, cell_style: { border_width: 0.1, borders: [:bottom]}) do
  ...
  row(-1).style font_style: :bold, borders: []
end
