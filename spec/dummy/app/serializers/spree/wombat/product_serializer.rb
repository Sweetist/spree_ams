...
module Spree
  module Wombat
    class ProductSerializer < ActiveModel::Serializer
      ...
      def cost_price
        object.current_cost_price.to_f rescue object.cost_price.to_f
      end
      ...
      def options_text
        object.option_types.where(vendor: object.vendor).pluck(:name)
      end

      def variants
        if object.variants.empty?
          [Spree::Wombat::VariantSerializer
            .new(object.master,
                 { root: false }.merge(options.except(:each_serializer, :root)))]
        else
