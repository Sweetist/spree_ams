$(document).ready ->
  ...
  $('a.save-line-item').click ->
    ...
    shipped_qty = parseInt(save.parents('tr').find('input.line_item_shipped_qty').val())
    ordered_qty = parseInt(save.parents('tr').find('input.line_item_ordered_qty').val())

    toggleItemEdit()
    adjustLineItem(line_item_id, quantity, shipped_qty, ordered_qty)
    false
